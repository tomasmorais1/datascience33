...
TIMESTAMP = "Timestamp"  # Nome da coluna de tempo no CSV gerado pelo scaling.py
...
def ts_aggregation_by(data, gran_level="D", agg_func="sum"):
    ...
    """
    df = data.copy()
    # Resample e aplica função (sum para contagens)
...
def evaluate_models(train, test):
    ...
    # Ajustar lags para datasets pequenos (ex: se agregarmos muito)
    n_lags = 3
...
def main():
    ...
    print(f"Dataset original (15min): {full_data.shape}")
    
    # Parametrizações:
    # Como tens dados de 15min, "H" (Hora) é uma agregação válida.
    # "D" (Dia) e "W" (Semana) também. "M" (Mês) é arriscado para apenas 2 meses de dados.
    granularities = ["H", "D", "W"] 
    agg_func = "sum" 
    ...
    if best_datasets[0] is not None:
        ...
        print(f"Saved to {OUTPUT_DATA_DIR}/aggregation_train.csv")
    else:
