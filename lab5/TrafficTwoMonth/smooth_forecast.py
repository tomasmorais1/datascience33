...
TARGET = "Total"
OUTPUT_DIR = Path("images/forecast_traffic_smooth")
...
# --------------------------------------------------------
# CUSTOM EVAL (Com Proteção MAPE e 2 Casas Decimais)
# --------------------------------------------------------
...
def run_linear_regression(series, gran_name):
    ...
    save_series = OUTPUT_DIR / f"traffic_lr_forecast_{gran_name.replace(' ', '_').replace('=', '')}.png"
...
def main():
    ...
    # 2. Generate "Forecasting plots after different smoothing" (SUBPLOTS)
    # Criar uma imagem única com todos os gráficos de linhas
    fig, axs = plt.subplots(len(WIN_SIZES), 1, figsize=(3 * HEIGHT, HEIGHT/1.5 * len(WIN_SIZES)))
    fig.suptitle(f"Traffic {TARGET} after smoothing")
    
    smoothed_series_dict = {}

    for i, size in enumerate(WIN_SIZES):
        # Calcular Smoothing e remover NaNs
        ss_smooth = hourly_ts.rolling(window=size).mean().dropna()
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "traffic_smoothing_plots.png")
    plt.close()
    print("-> Saved smoothing comparison plot.")

    # 3. Run LR on each smoothed series ("Forecasting results after...")
    # Também corremos na original para comparar
    run_linear_regression(hourly_ts, "Hourly Raw")
