...
TARGET = "Total"
OUTPUT_DIR = Path("images/forecast_traffic_diff")
...
def custom_plot_eval(train, test, prd_trn, prd_tst, title):
    ...
    def calc_metrics(y_true, y_pred):
        ...
        # 1. Filtra zeros absolutos para não dividir por zero
        mask = np.abs(y_true) > 1e-5 
        
        if np.sum(mask) > 0:
            # Calcula erro percentual apenas onde y_true != 0
            pe = np.abs((y_true[mask] - y_pred[mask]) / y_true[mask])
            mape = np.mean(pe)
        else:
            mape = 0.0

        # 2. Se mesmo assim der um valor gigante (ex: 1e13), força a 0
        # Isso acontece quando temos valores muito pequenos tipo 0.0000001
        if mape > 1000: 
            mape = 0.0
...
def run_linear_regression(series, gran_name):
    ...
    save_eval = OUTPUT_DIR / f"traffic_lr_eval_{gran_name.lower().replace(' ', '_')}.png"
    ...
    save_series = OUTPUT_DIR / f"traffic_lr_forecast_{gran_name.lower().replace(' ', '_')}.png"
...
# --------------------------------------------------------
def main():
    print("\n=== DIFFERENTIATION & LR (TRAFFIC - HOURLY) ===\n")
    ...
    # 2. Differentiations
    diff1_ts = hourly_ts.diff().dropna()
    diff2_ts = diff1_ts.diff().dropna()

    # 3. Plots Differentiation
    plt.figure(figsize=(3 * HEIGHT, HEIGHT))
    plot_line_chart(diff1_ts.index, diff1_ts.values, 
                   title="Traffic - 1st Differentiation (Hourly)",
                   xlabel="Date", ylabel="Change")
    plt.savefig(OUTPUT_DIR / "traffic_plot_1st_diff.png")
    plt.close()
    
    plt.figure(figsize=(3 * HEIGHT, HEIGHT))
    plot_line_chart(diff2_ts.index, diff2_ts.values, 
                   title="Traffic - 2nd Differentiation (Hourly)",
                   xlabel="Date", ylabel="Change in Change")
    plt.savefig(OUTPUT_DIR / "traffic_plot_2nd_diff.png")
    plt.close()

    # 4. Run LR
    series_to_test = {
        "Hourly Raw": hourly_ts,
        "1st Diff": diff1_ts,
        "2nd Diff": diff2_ts
    }
