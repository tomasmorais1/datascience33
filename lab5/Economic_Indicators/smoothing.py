#!/usr/bin/env python3
"""
Lab 5 – SMOOTHING PREPARATION (MACRO DATA)
...
from dslabs_functions import plot_bar_chart, plot_line_chart # Assumindo que estas funções estão disponíveis
...
def apply_smoothing(data, win_size):
    ...
    df = data.copy()
    if win_size > 1:
        # Aplica a suavização ( Rolling Mean )
...
def main():
    ...
    for win in win_sizes:
        ...
        # Combinamos o plot de treino e teste
        ts_to_plot = pd.concat([train_sm, test_sm])

        plot_line_chart(
            ts_to_plot.index,
            ts_to_plot[TARGET],
            title=f"{TARGET} - Smoothing window={win}",
            xlabel="Time",
            ylabel=TARGET
        )
        
        # Adicionar linha vertical para marcar o split
        plt.axvline(split_date, color='red', linestyle='--', label='Train/Test Split')
        
        plt.legend()
        plt.tight_layout()
        plt.savefig(OUTPUT_IMG_DIR / f"smoothing_inflation_win_{win}.png")
        plt.close()
    ...
    if valid_results:
        plot_bar_chart(list(valid_results.keys()), list(valid_results.values()),
                       title="RMSE by Smoothing Window (Inflation Rate)",
                       xlabel="Window Size", ylabel="RMSE")
        plt.savefig(OUTPUT_IMG_DIR / "smoothing_comparison_inflation.png")
    plt.close()
