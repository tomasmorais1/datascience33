#!/usr/bin/env python3
"""
Lab 5 – SCALING PREPARATION (MACRO DATA)
Target: Inflation Rate (%) for USA
"""
import pandas as pd
import numpy as np
...
from dslabs_functions import plot_bar_chart # Assumindo que esta função está disponível

# --- CONFIGURAÇÃO ---
DATAFILE = "economic_indicators_dataset_2010_2023.csv"
TARGET = "Inflation Rate (%)" # A coluna que queremos prever
COUNTRY_FILTER = "USA"       # País a filtrar
...
def prepare_data():
    """
    Lê o csv, filtra pelo país (USA), usa a coluna 'Date' como índice 
    e seleciona o TARGET (Inflation Rate %).
    """
    print(f"Loading {DATAFILE}...")
    try:
        # Carregar com parse_dates para usar a coluna 'Date' como datetime
        df = pd.read_csv(DATAFILE, parse_dates=['Date'])
    except FileNotFoundError:
        print(f"ERRO: Ficheiro {DATAFILE} não encontrado.")
        exit()

    # 1. FILTRAR E INDEXAR (Alteração Chave)
    print(f"Filtering for {COUNTRY_FILTER} and setting 'Date' as index...")
    
    # Filtrar pelo país e remover NaNs antes de continuar
    ...
    # Usamos a coluna 'Date' real como índice
    df_filtered = df_filtered.set_index('Date').sort_index()

    # 2. SELEÇÃO DO TARGET
    if TARGET not in df_filtered.columns:
        raise ValueError(f"Target '{TARGET}' não encontrado. Colunas disponíveis: {df_filtered.columns.tolist()}")

    # Selecionamos apenas a coluna alvo para a série temporal e removemos NaNs
    ts = df_filtered[[TARGET]].astype(float).dropna()
    return ts
...
def evaluate_models(train, test, scaler=None):
    ...
    As métricas são calculadas na escala original (Inverse Transform).
    """
    # 1. Persistence (Previsão t = valor em t-1)
    # NÃO É USADO PARA AVALIAÇÃO NESTE CÓDIGO, MAS PODERIA SER
    ...
    # Garantir que o conjunto de teste tem dados suficientes após criar lags
    if len(test_lags) == 0:
    ...
    if scaler:
        # A transformação inversa requer um array 2D
...
def main():
    print(f"\n=== LAB 5: SCALING PREPARATION (Target: {TARGET} for {COUNTRY_FILTER}) ===\n")
    ...
    for method in methods:
        ...
        plt.title(f"{TARGET} ({COUNTRY_FILTER}) Scaling: {method}")
    ...
    # 5. Guardar datasets para a próxima tarefa
