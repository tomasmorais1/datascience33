...
from dslabs_functions import plot_bar_chart, plot_line_chart, HEIGHT
...
def main():
    ...
    # Podes ajustar estes valores. win=1 é o "Original" (sem suavização).
    # win=10 significa média das últimas 10 horas/dias/semanas.
    win_sizes = [1, 5, 10, 20]
    ...
    if valid_results:
        plot_bar_chart(list(valid_results.keys()), list(valid_results.values()), 
                       title="Linear Regression RMSE by Smoothing Window", 
    ...
    if best_datasets[0] is not None:
        ...
        # Guardamos como smoothing_train.csv para consistência
