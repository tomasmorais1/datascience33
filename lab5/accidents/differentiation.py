...
def main():
    ...
    for order in diff_orders:
        ...
        plt.plot(train_diff.index, train_diff[TARGET], label="Train")
        plt.plot(test_diff.index, test_diff[TARGET], label="Test")
        plt.title(f"Differentiation: Order {order}")
        plt.legend()
        plt.tight_layout()
        plt.savefig(OUTPUT_IMG_DIR / f"differentiation_{name}.png")
        plt.close()
    ...
    plot_bar_chart(list(results.keys()), list(results.values()), 
                   title="Linear Regression RMSE by Differentiation", 
                   xlabel="Diff Order", ylabel="RMSE")
    plt.savefig(OUTPUT_IMG_DIR / "differentiation_comparison.png")
    plt.close()
