...
# ==========================================================
# CARREGAR DATASET + LIMITAR A 50K LINHAS
# ==========================================================
data: DataFrame = read_csv(filename, index_col=index_col, na_values="")

# Limitar número de registos a 50.000
sample_size = 50000
if data.shape[0] > sample_size:
    data = data.sample(n=sample_size, random_state=42)

print(f"Dataset carregado com {data.shape[0]} linhas (limitado).")
...
summary5.to_csv(f"images/{file_tag}_summary.csv")
...
def compute_known_distributions_safe(values: list) -> dict:
    ...
    try:
        if all(v > 0 for v in values):
            ...
            distributions[f"LogNor({log(scale):.1f},{sigma:.2f})"] = lognorm.pdf(x, sigma, loc, scale)
    except Exception:
        pass
